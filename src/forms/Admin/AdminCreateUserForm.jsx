...
function AdminCreateUserForm({user}) {
    ...
    async function handleSubmit(e) {
        ...
        const handleErrors = async () => {
            ...
            if (!validTopDomains.includes('.'+ formData.email.split('.')[1])) {
                setFlashMsg({...flashMsg, emailNotValid: 'Email must be a valid email!'});
                isErr = true;
            } 
        }
        handleErrors()
    ...
    return (
        ...
        {flashMsg.errors && flashMsg.errors.map(err=> (<p className="FlashMsg-error" key={err}>{err}</p>)) }
        {user && 
        <Form className="form">
          <FormGroup>
          ...
          <FormGroup>
            {flashMsg.emailNotValid && <p className="FlashMsg-error">{flashMsg.emailNotValid}</p>}
          ...
          <FormGroup>
            {flashMsg.passwordMatch && <p className="FlashMsg-error">{flashMsg.passwordMatch}</p>}
          ...
          <Button type="submit" onClick={handleSubmit} disabled={!isComplete()}>Submit</Button>
          {modalState && <SuccessModal setModalState={setModalState} formData={formData} modalState={modalState} modalMessage={`Successfully created User: ${formData.username}`}/>}
          </Form>
        }
        </div>
    )
